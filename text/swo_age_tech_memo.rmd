...
[Objective para - Pete and Matt] The main objectives of this study were to (1) document methods for expanding length and age compositions, (2) present species-specific results of historical input samples size for Tier $\geq$3 stocks, and (3) record methods for estimating input sample sizes of survey-based age and length compositions using a two-stage bootstrapping approach.
...
We applied the bootstrap-simulation to species that are assessed in the EBS, AI, and GOA with statistical catch-at-age models (or are Tier $\geq$3, *table(s)?*).
